...
$(document).ready(() => {
    ...
    $("fieldset input:not(#present_AddressLine2,#permanent_AddressLine2),select").each((index, inputElement) => {
...
async function populateUserDetails(data) {
    console.log(data);
    await populateCountryList();
    $("[type='text'],[type='password'],[type='date'],select").each(async (index, item) => {
        if (item.id.includes("State")) {
            await populateStates(item.id.replace("State", "Country"));
        }
...
function populatePermanentAsPresent() {
    if ($("#ifPresentSameAsPermanent").prop("checked")) {
        $("#permanentAddress input,select").each(async (index, item) => {
            if (item.id == "permanent_State") {
                await populateStates("permanent_Country")
            }
...
async function populateCountryList() {
    let optionList = await fetchCountries();
    optionList.forEach((option) => {
        $("select[id*=Country]").append(generateFunc(option.countryId,option.countryName));
    });
}
...
function generateDocumentDiv(document) {
    ...
    return `<div class="document-element">
            ...
            <div class="download-icon"><i class="fa-regular fa-circle-down"></i></div>
        </div>`;
}
...
function resetDocumentFile() {
    $('#documentFile').val('');
    $("#documentName").text('');
    $('#iconForAddDocument').attr("class",'fa-sharp fa-solid fa-plus');
}
